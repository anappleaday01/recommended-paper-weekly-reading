...
```mermaid
graph TD
    A[ADHD的大样本元分析: 默认模式网络假说的证据] --> B[核心目标与发现]
    B --> B1[目标: 通过大样本元分析检验ADHD的默认模式网络干扰假说]
    B --> B2[发现: ADHD与DMN和任务正向网络间的反连接减弱相关<br>• 效应量小但稳健<br>• 支持ADHD脑表型的连续性]

    A --> C[研究背景与动机]
    C --> C1[1. ADHD的脑网络假说]
    C1 --> C11[默认模式网络干扰假说: DMN与任务网络互扰致注意缺损]
    C1 --> C12[既往研究结果不一致: 样本小, 方法异质, 混淆多]
    C2 --> C21[元分析的局限: 依赖已发表结果, 有发表偏倚, 无法控制个体水平混淆]
    C3 --> C31[大样本数据的优势: 减少误差, 可严格匹配和控制混淆]

    A --> D[研究设计与方法]
    D --> D1[1. 样本与数据]
    D1 --> D11[病例-对照分析: ADHD-200, HBN, ABCD, NCR (N=2602)]
    D1 --> D12[特质分析: HBN, ABCD, NCR, NCANDA, HCP-D (N=10,113)]
    D2 --> D21[预处理: 标准化36参数+去尖峰管道, 严格头动控制]
    D3 --> D31[连接分析: Schaefer 400分区, 7个网络, 计算网络内/间连接]
    D4 --> D41[统计: 线性混合效应模型, 双调整法控制协变量, FDR校正]
    D5 --> D51[敏感性分析: 控制共病内化/外化问题, 药物, 头动, 诊断标准]

    A --> E[主要结果]
    E --> E1[1. 病例-对照差异 (N=2602)]
    E1 --> E11[DMN与背侧注意网络: 反连接减弱 (d=0.17)]
    E1 --> E12[DMN与凸显/腹侧注意网络: 反连接减弱 (d=0.14)]
    E1 --> E13[DMN与躯体运动网络: 反连接减弱 (d=0.14)]
    E2 --> E21[DMN与背侧注意网络: 关联最强 (partial-r=0.06)]
    E2 --> E22[DMN与凸显/腹侧注意, 额顶网络关联也显著]
    E3 --> E31[结果对共病问题, 药物, 严格头动阈值, 诊断标准稳健]

    A --> F[讨论与意义]
    F --> F1[1. 支持默认模式干扰假说]
    F1 --> F11[反连接减弱与外向注意任务表现受损和思维游移相关]
    F2 --> F21[病例对照与特质分析结果高度相似, 支持ADHD连续体模型]
    F3 --> F22[大样本元分析优于传统元分析: 可控制关键混淆]
    F3 --> F23[效应量小: ADHD病理生理复杂, 脑连接异常仅为一部分]
    F3 --> F24[未来方向: 多变量预测模型, 累积风险评分, 纵向动态研究]
