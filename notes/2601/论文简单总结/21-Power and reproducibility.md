...
```mermaid
graph TD
    A[脑表型预测模型外部验证的统计功效与可重复性] --> B[核心目标与发现]
    B --> B1[核心问题: 外部验证的统计功效如何取决于训练与外部样本量?]
    B --> B2[主要发现: <br>• 外部验证功效取决于训练和外部样本量<br>• 既往研究普遍样本量不足(功效低)<br>• 内部验证表现可提示外部验证表现]

    A --> C[研究背景与动机]
    C --> C1[1. 神经影像预测模型的泛化挑战]
    C1 --> C11[内部验证(交叉验证)无法完全保证模型泛化性]
    C1 --> C12[外部验证是评估泛化性的黄金标准]
    C1 --> C13[但外部数据集常存在“数据集偏移”]
    C2 --> C21[传统功效计算仅考虑单一样本量]
    C2 --> C22[外部验证涉及两个样本量, 传统方法不适用]
    C3 --> C31[领域现状: 外部验证研究样本量中位数约100人]

    A --> D[研究方法: 大规模重采样模拟]
    D --> D1[1. 数据集与表型]
    D1 --> D11[功能连接: HBN, ABCD, HCPD, PNC (美国青少年)]
    D1 --> D12[结构连接: HBN, HCPD, QTAB, CHCP, HCP (扩展至成人与跨国样本)]
    D1 --> D13[表型: 年龄, BMIz, 矩阵推理, 工作记忆, 注意问题, 焦虑抑郁症状 (按效应大小分高/中/低)]
    D2 --> D21[模型: 岭回归, 控制协变量, 特征选择(前1%)]
    D3 --> D31[重采样设计: 系统变化训练样本量和外部样本量, 各组合重复100次]
    D4 --> D32[评价指标: 预测值与真实值的Pearson's r, 平均绝对误差]
    D5 --> D33[核心计算: 模拟功效, 假阳性率, 效应量膨胀/收缩]

    A --> E[主要结果 (功效与样本量)]
    E --> E1[1. 外部验证功效同时受训练和外部样本量影响]
    E1 --> E11[高效应表型: 数百样本可达高功效]
    E1 --> E12[中/低效应表型: 需要数百至数千样本]
    E2 --> E21[小训练样本 + 大外部样本易产生假阳性]
    E3 --> E31[小外部样本易导致效应量膨胀]
    E4 --> E41[理论功效曲线需根据训练样本量(影响真实r)调整]

    A --> F[主要结果 (内部与外部表现关系)]
    F --> F1[1. 内部验证表现是外部表现的实用指标]
    F1 --> F11[内部与外部验证的r差异通常<0.2 (尤其在数据协调良好时)]
    F1 --> F12[内部验证显著的模型, 外部验证也更容易显著]
    F2 --> F21[数据集偏移增大内外表现差异 (如跨国样本)]

    A --> G[讨论与建议]
    G --> G1[1. 对研究实践的建议]
    G1 --> G11[样本量: 训练集和外部集都应尽可能大]
    G1 --> G12[效应量: 报告效应大小与p值, 警惕小样本下的膨胀]
    G1 --> G13[指导原则: 如果一个数据集样本量小到你不愿用它训练模型, 那你也不应仅用它做外部验证]
    G2 --> G21[2. 提升功效的潜在途径]
    G2 --> G22[增加个体数据量: 延长扫描时长对内部验证有益, 但对提升外部验证功效有限]
    G2 --> G23[减少数据集偏移: 协调测量工具, 使用多样化代表性样本]
    G3 --> G31[3. 领域发展方向]
    G3 --> G32[鼓励发表非显著的外部验证结果]
    G3 --> G33[外部验证有助于解决机器学习中的偏见和可信度问题]
