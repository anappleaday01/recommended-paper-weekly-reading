...
```mermaid
graph TD
    A[域适应的脑网络融合捕捉青春期脑发育与心理健康变异] --> B[核心目标与发现]
    B --> B1[核心问题: 如何构建稳定、可迁移的低维表征来刻画青春期脑发育?]
    B --> B2[主要发现: <br>• 提出融合SNF与域适应的ML框架，得到稳定“脑嵌入”分<br>• 该分能敏感捕捉青春期成熟相关脑变异（尤其女性）<br>• 该分与精神病理学严重程度相关（尤其女性）]

    A --> C[研究背景与动机]
    C --> C1[1. 青春期脑发育的复杂性与异质性]
    C1 --> C11[青春期是脑重组、心理健康风险升高的关键期]
    C1 --> C12[需要能刻画个体发育轨迹的稳定计算表型]
    C2 --> C21[多模态数据融合的挑战: 维度灾难、有效整合]
    C2 --> C22[相似性网络融合的优势与局限: 信息整合好，但新样本加入会改变相似性结构，得分不稳定]
    C3 --> C23[目标: 克服SNF不稳定性，构建可迁移、稳定的低维脑发育表型]

    A --> D[方法框架: 三步骤]
    D --> D1[1. 构建参考空间（PNC数据集）]
    D1 --> D11[数据: PNC (N=1594, 8-21岁)， 皮层68区面积与体积]
    D1 --> D12[步骤: SNF融合双模态 -> 扩散映射降维 -> 提取“第一脑嵌入”作为参考分]
    D2 --> D21[2. 监督式域适应学习映射]
    D2 --> D22[模型: 弹性网络 + TrAdaBoostR2（域适应算法）]
    D2 --> D23[训练: 用PNC（源域）+ ABCD/HBN部分数据（目标域）训练，学习从原始特征到脑嵌入分的映射]
    D3 --> D31[3. 预测新数据脑嵌入分]
    D3 --> D32[应用: 在未见过的ABCD纵向数据和HBN临床数据上预测脑嵌入分]
    D4 --> D33[验证: 预测分与在目标数据集上重新计算SNF得到的“真实”分高度相关（R²: ABCD ~0.79, HBN ~0.65）]

    A --> E[主要结果1: 捕捉青春期脑发育]
    E --> E1[1. 横断面关联]
    E1 --> E11[ABCD数据: 脑嵌入分与PDS评分显著负相关（看护者报告效应更强）]
    E1 --> E12[关联程度大于年龄效应，提示其对青春期特异]
    E1 --> E13[控制BMI、SES、种族后，横断面关联减弱]
    E2 --> E21[2. 纵向变化关联]
    E2 --> E22[Δ脑嵌入分与ΔPDS评分显著相关（仅女性显著，看护者与青少年报告均显著）]
    E2 --> E23[控制基线青春期状态后，效应更强]
    E2 --> E24[Δ脑嵌入分在青春期早期性别分布相似，女性初潮后开始分化]

    A --> F[主要结果2: 关联精神病理学]
    F --> F1[1. 临床样本（HBN）]
    F1 --> F11[脑嵌入分与诊断数量总和（疾病严重度代理）显著正相关（仅女性）]
    F1 --> F12[使用维度量表（CBCL总分）重复出相似结果（仅女性）]
    F1 --> F13[控制青春期变量后，精神病理学效应仍显著]
    F2 --> F21[2. 社区样本（ABCD）验证]
    F2 --> F22[脑嵌入分与CBCL总分显著相关（所有模型及男性纵向模型）]

    A --> G[讨论与意义]
    G --> G1[1. 方法贡献]
    G1 --> G11[解决了SNF得分不稳定的问题，实现跨数据集稳定映射]
    G1 --> G12[结合监督域适应，增强了模型对新数据集的泛化能力]
    G2 --> G21[2. 科学发现]
    G2 --> G22[脑嵌入分是对青春期脑动态敏感的表型，尤其适用于女性]
    G2 --> G23[该分也与精神病理学相关，可能作为“偏离常态”的指标]
    G3 --> G31[3. 局限与未来方向]
    G3 --> G32[仅使用两个结构模态（面积、体积），未来可扩展]
    G3 --> G33[空间可解释性有限，需结合其他方法（如特征相关图）]
    G3 --> G34[样本量限制，尤其在HBN中青春期关联未重复]
